...
export default function EditorSidebar({ inv, setInv, cfg, update, setPreviewAnim, mobileView }) {
  ...
  const generateAIPhrase = () => {
    const phrases = [
      ...
      "Un momento especial merece ser compartido.",
      "Hay momentos inolvidables, vení a vivirlos con nosotros.",
      "¡Prepará tu mejor outfit porque se viene un fiestón!",
      "El mejor regalo es tu presencia. ¡No faltes!",
      "Brindemos juntos por los buenos momentos que vendrán.",
      "Llegó el gran día y queremos que seas parte."
    ];
    const random = phrases[Math.floor(Math.random() * phrases.length)];
    update("eventType", random);
  };
  ...
  return (
    <aside className={`w-[100vw] md:w-[420px] h-full shrink-0 bg-[#f8f7ff] overflow-y-auto p-6 pb-24 md:pb-6 border-r border-gray-100 z-10 fd-sb ${mobileView === 'editor' ? 'block' : 'hidden md:block'}`}>
      ...
      <Acc title="🎨 Diseño Base y Animación" icon={Palette} iconColor="#6366f1">
        ...
        <div className="mb-6 p-4 rounded-xl border border-pink-100 bg-pink-50/50">
          <div className="flex items-center justify-between mb-4">
            <span className="text-[10px] font-black text-pink-600 uppercase tracking-widest flex items-center">Bordes Ornamentales <Tooltip text="Añade esquineros decorativos a la foto principal." /></span>
          ...
          <div className="flex items-center justify-between mb-4 bg-white p-2 rounded-lg border border-pink-100">
            ...
            <Toggle checked={cfg.showCoverBorders || false} onChange={v => { update("showCoverBorders", v); if (v && !cfg.selectedBorder) update("selectedBorder", "/borders/1-Photoroom.png"); }} />
          </div>
        ...
        <div className="mb-6 p-3 bg-violet-50 rounded-xl border border-violet-100 shadow-inner">
          <label className="flex justify-between items-center text-[9px] font-black text-violet-600 uppercase mb-2">
            <span className="flex items-center">Luz / Sombra Tarjetas</span>
            <span className={`px-2 py-0.5 rounded-full ${cfg.cardGlow === 0 || cfg.cardGlow === undefined ? 'bg-slate-200 text-slate-600' : 'bg-violet-200 text-violet-800'}`}>{(cfg.cardGlow === undefined || cfg.cardGlow === 0) ? "SIN" : `${cfg.cardGlow}%`}</span>
          </label>
      ...
      <Acc title="1️⃣ Portada Principal" icon={ImageIcon} defaultOpen iconColor="#ec4899">
        ...
        <div className="z-[80] relative mt-4"><Inp label="Nombre Principal" value={cfg.honoreeName} onChange={v => update("honoreeName", v)} /></div>
        ...
        <div className="flex items-center justify-between mb-4 border-t border-gray-100 pt-4"><span className="text-xs font-bold text-slate-500 flex items-center">Mostrar Medalla Flotante</span><Toggle checked={cfg.showBadge ?? true} onChange={v => update("showBadge", v)} /></div>
      ...
      <Acc title="4️⃣ Cuándo y Dónde" icon={Calendar} iconColor="#e11d48">
         ...
         <div className="flex items-center justify-between mt-4 mb-2 border-t border-gray-100 pt-4"><span className="text-xs font-bold text-slate-500 flex items-center">Ubicación</span><Toggle checked={cfg.showLocation} onChange={v => update("showLocation", v)} /></div>
         {cfg.showLocation && (
           <>
      ...
      <Acc title="5️⃣ Tarjeta del Salón" icon={LinkIcon} iconColor="#6366f1">
        <div className="flex items-center justify-between mb-4">
          <span className="text-xs font-bold text-slate-500 flex items-center">Mostrar Tarjeta</span>
      ...
      <Acc title="9️⃣ Vestimenta y Regalos" icon={Layout} iconColor="#f43f5e">
         ...
         <div className="flex items-center justify-between mb-4 pt-4 border-t"><span className="text-xs font-bold text-slate-500 flex items-center">Activar Regalos</span><Toggle checked={cfg.showGifts} onChange={v => update("showGifts", v)} /></div>
         {cfg.showGifts && (
           <>
      ...
      <Acc title="📸 Álbum en Vivo" icon={Camera} iconColor="#0ea5e9">
        <div className="flex items-center justify-between mb-4">
          ...
          <Toggle checked={cfg.showLiveCamera || false} onChange={v => update("showLiveCamera", v)} />
        </div>
        {cfg.showLiveCamera && (
      ...
      <Acc title="1️⃣1️⃣ Confirmación y Redes" icon={CheckCircle2} iconColor="#22c55e">
        <div className="bg-slate-50 p-4 rounded-xl border mb-6">
           ...
           <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-bold text-slate-700 uppercase flex items-center">Modo Lista VIP (Nominal)</span>
           ...
           {cfg.isPrivateList && (
             <>
               <div className="mb-4 p-3 bg-violet-50 rounded-xl border border-violet-100">
                 <label className="flex items-center text-[10px] font-black text-violet-800 uppercase tracking-widest mb-1.5">PIN de Acceso Agasajado</label>
           ...
           <div className="flex items-center justify-between mb-2 border-t border-slate-200 pt-4">
              <span className="text-[10px] font-bold text-slate-700 uppercase flex items-center">Fecha Límite RSVP</span>
           ...
           <h4 className="flex items-center text-[10px] font-bold text-slate-700 uppercase mb-2 border-t border-slate-200 pt-4">Límite Acompañantes</h4>
        ...
        <div className="space-y-4">
          <div className="bg-slate-50 p-3 rounded-xl border">
            <div className="flex items-center justify-between mb-2">
               ...
               <Toggle checked={cfg.showInstagram || false} onChange={v => { if (v && salonProfile?.instagram && !cfg.instagramUrl) { setInv(prev => ({ ...prev, config: { ...prev.config, showInstagram: true, instagramUrl: salonProfile.instagram }})); } else { update("showInstagram", v); } }} />
            </div>
          ...
          <div className="bg-slate-50 p-3 rounded-xl border">
            <div className="flex items-center justify-between mb-2">
               ...
               <Toggle checked={cfg.showFacebook || false} onChange={v => { if (v && salonProfile?.facebook && !cfg.facebookUrl) { setInv(prev => ({ ...prev, config: { ...prev.config, showFacebook: true, facebookUrl: salonProfile.facebook }})); } else { update("showFacebook", v); } }} />
            </div>
          ...
          <div className="bg-slate-50 p-3 rounded-xl border">
            <div className="flex items-center justify-between mb-2">
               ...
               <Toggle checked={cfg.showTiktok || false} onChange={v => { if (v && salonProfile?.tiktok && !cfg.tiktokUrl) { setInv(prev => ({ ...prev, config: { ...prev.config, showTiktok: true, tiktokUrl: salonProfile.tiktok }})); } else { update("showTiktok", v); } }} />
            </div>
