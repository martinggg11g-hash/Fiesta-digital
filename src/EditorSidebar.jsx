...
import {
  ...
  MessageCircle, Plus, Edit2, RefreshCcw
} from "lucide-react";
      ...
      <Acc title="🎨 Diseño Base y Animación" icon={Palette} iconColor="#6366f1">
        ...
        <div className="mb-6 p-4 rounded-xl border border-pink-100 bg-pink-50/50">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={resetPositions} title="Centrar todo lo arrastrable" className="p-2 bg-white border border-pink-200 text-pink-600 rounded-lg hover:bg-pink-100 cursor-pointer">
              <RefreshCcw size={14} />
            </button>
          </div>
        ...
        {(cfg.showBadge ?? true) && (
          <>
            ...
            <div className="mt-4 p-3 bg-violet-50 rounded-xl border border-violet-100">
              ...
              <input type="color" value={cfg.badgeBgColor || "#000000"} onChange={e => update('badgeBgColor', e.target.value)} className="w-full h-10 rounded-xl cursor-pointer border-none"/>
            </div>
          </>
        )}
      </Acc>
