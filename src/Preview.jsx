...
const RsvpWidget = ({ cfg, primary, textC, cardC, onConfirmRSVP }) => {
  ...
  if(step === 'button') return <button onClick={()=>setStep('form')} className="w-full py-5 rounded-2xl font-black shadow-xl text-white transition-all active:scale-95" style={{ background: primary }}>OBTENER PASE VIP</button>;
  if(step === 'form') return (
    <form onSubmit={generateTicket} className="p-6 rounded-3xl bg-white/10 border border-white/10 space-y-4">
      <input type="text" placeholder="Nombre" className="w-full p-4 rounded-xl bg-black/20 text-white outline-none" onChange={e=>setFormData({...formData, name: e.target.value})} required />
      <input type="text" placeholder="Apellido" className="w-full p-4 rounded-xl bg-black/20 text-white outline-none" onChange={e=>setFormData({...formData, lastname: e.target.value})} required />
      <button type="submit" disabled={loading} className="w-full py-4 bg-white text-black font-black rounded-xl">{loading ? 'Procesando...' : 'GENERAR PASE'}</button>
    </form>
  );
  return (
    <div className="text-center p-6 bg-white/5 rounded-3xl border border-white/10">
      <img src={ticketImage} className="w-full max-w-[240px] mx-auto rounded-xl shadow-2xl mb-4" />
      <a href={ticketImage} download="Pase_VIP.jpg" className="block w-full py-3 bg-green-500 text-white font-black rounded-xl">DESCARGAR IMAGEN</a>
    </div>
  );
};
