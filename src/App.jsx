...
import {
  ...
  LogOut, UserPlus, Plus, Users, Trash2, Edit2, Copy, X, CheckCircle2, Key, Info, BellRing
} from "lucide-react";

// Función auxiliar para crear URLs amigables
const slugify = (text) => text?.toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, '') || 'salon';

/* ============================================================================
   MICRO COMPONENTES UI (Necesarios para el panel)
============================================================================ */
...
/* ============================================================================
   2. PANTALLA DASHBOARD (MASTER Y SALONES)
============================================================================ */
export const DashboardScreen = ({ user, onLogout, users, onUpdateUser, onCreateSalon, invitations, onCreateInv, onDeleteInv, onUpdateInternal }) => {
  ...
  if (!user) {
    // Usamos setTimeout para evitar advertencias de React Router al renderizar
  ...
  const toggleAlert = (u) => {
    ...
    notify(u.paymentAlert ? "Alerta quitada" : "Alerta de pago activada");
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8f7ff]">
      {/* Alerta de pago para el cliente */}
      {amIAlerted && (
      ...
      <div className="max-w-6xl mx-auto p-6 sm:p-12">
        ...
        {isOwner ? (
          ...
          /* ================= VISTA SALÓN: LISTA DE INVITACIONES ================= */
      ...
      {/* Modal Cambiar Contraseña (Salón) */}
      {showPassModal && (
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-md flex items-center justify-center p-6 z-[100]">
