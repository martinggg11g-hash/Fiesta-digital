...
import {
  ...
  Phone, Users, Utensils, Music, CreditCard, Clock, Settings, UserCheck, Receipt,
  Moon, Sun, Printer, ClipboardList, ImageIcon, FileText, ScanBarcode, FileDown
} from "lucide-react";
...
// MÓDULO INTELIGENTE DE CÁMARA QR
...
export const LoginScreen = ({ isMaster = false, onLogin, users }) => {
  ...
  const handleAuth = (e) => {
    ...
    setTimeout(() => {
      ...
      if (found) { onLogin(found, rememberMe); navigate("/dashboard"); } else setError("Credenciales no válidas.");
      setLoading(false);
    }, 500);
  };
...
export const DashboardScreen = ({ user, onLogout, users, onUpdateUser, onCreateSalon, onDeleteSalon, invitations, onCreateInv, onDeleteInv, onUpdateInternal }) => {
  ...
  const [activeTab, setActiveTab] = useState("info"); // 'info' | 'guests'
  
  const [showSettings, setShowSettings] = useState(false);
  const [scanningEvent, setScanningEvent] = useState(null); // Almacena el evento al que le estamos escaneando la puerta
  const [validationResult, setValidationResult] = useState(null); // Modal de resultado del escaneo
  ...
  const processQRScan = (qrString) => {
    // qrString format: PASS-ID|Name|Lastname|Pax
            ...
            <div className="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4">
              <div className={`w-full max-w-4xl max-h-[95vh] h-full sm:h-auto rounded-[2rem] overflow-hidden flex flex-col shadow-2xl anim-pop ${isDark ? 'bg-slate-900' : 'bg-white'}`}>
                ...
                <div className="p-6 sm:p-8 overflow-y-auto fd-sb flex-1">
                  ...
                  {/* NUEVA PESTAÑA: LISTA DE INVITADOS */}
                  {activeTab === 'guests' && (
...
// PANTALLA PÚBLICA (RECIBE ONCONFIRMRSVP)
  ...
  return (
    <div className="bg-black min-h-screen flex justify-center w-full relative overflow-hidden">
      ...
      <div className={`w-full max-w-[480px] bg-white shadow-2xl relative transition-opacity duration-1000 ${opened ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden'}`}>
        <InvitePreview cfg={inv.config} status={inv.internal_data?.eventStatus} onConfirmRSVP={(guestData) => onConfirmRSVP(inv.id, guestData)} />
      </div>
    </div>
  );
};
...
export default function App() {
  ...
  const handleUpdateUser = async (email, updateData) => {
    ...
    if (salonError) return alert("Error en la Base de Datos: " + salonError.message);
  ...
  const handleSaveInv = async (updatedInv) => {
    const { error } = await supabase.from('invitaciones').update({ 
      title: updatedInv.title, config: updatedInv.config, internal_data: updatedInv.internal_data 
    }).eq('id', updatedInv.id);
  ...
  const handleUpdateInternal = async (id, field, val) => {
    setInvitations(prev => prev.map(i => {
      if (i.id === id) { return { ...i, internal_data: { ...i.internal_data, [field]: val } }; }
      return i;
    }));
  ...
  // FUNCIÓN MAESTRA QUE GUARDA AL INVITADO EN SUPABASE
